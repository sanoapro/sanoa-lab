...
export async function GET(req: NextRequest, { params }: { params: { id: string } }) {
  ...
  try {
    ...
    let q = supa.from("v_patients").select("id, org_id, name, gender, dob, tags, created_at, deleted_at").eq("id", params.id);
    ...
    const { data: labels } = await supa.from("patient_labels").select("label").eq("org_id", data.org_id).eq("patient_id", data.id);

    return NextResponse.json({ ok: true, data: { ...data, labels: (labels ?? []).map(l => l.label) } });
  } catch (e: any) {
