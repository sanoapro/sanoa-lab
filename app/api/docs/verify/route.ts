...
export async function GET(req: Request) {
  ...
  if (type === "prescription") {
    ...
    const { data } = await svc.from("referrals").select("created_at").eq("id", id).maybeSingle();
    created_at = data?.created_at || null;
  } else if (type === "discharge") {
    const { data } = await svc.from("discharges").select("created_at").eq("id", id).maybeSingle();
    created_at = data?.created_at || null;
  } else if (type === "lab_request") {
    const { data } = await svc.from("lab_requests").select("created_at").eq("id", id).maybeSingle();
    created_at = data?.created_at || null;
  }
