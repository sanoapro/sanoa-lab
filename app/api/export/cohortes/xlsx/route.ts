...
export async function GET(req: NextRequest) {
  ...
  ["from","to","cohort"].forEach(k => { const v = url.searchParams.get(k); if (v) params.set(k, v); });

  const origin = url.origin;
  const r = await fetch(`${origin}/api/reports/cohorts?${params.toString()}`, { cache:"no-store" });
