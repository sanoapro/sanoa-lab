...
export async function GET(req: NextRequest) {
  ...
  if (tagIds.length > 0) {
    ...
    const { data: rows, error } = await supabase.rpc("patients_ids_by_tags", { tag_ids: tagIds, mode });
    ...
    if (idsByTags.length === 0) {
      ...
      return new NextResponse(csvEmpty, {
        headers: { "content-type": "text/csv; charset=utf-8", "content-disposition": "attachment; filename=pacientes.csv" },
      });
    }
  }
