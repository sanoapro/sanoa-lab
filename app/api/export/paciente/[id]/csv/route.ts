...
export async function GET(_: NextRequest, { params }: { params: { id: string } }) {
  ...
  const [{ data: p, error: e1 }, { data: notes, error: e2 }, { data: appts, error: e3 }, { data: pts, error: e4 }] =
    await Promise.all([
      supabase.from("v_patients_export").select("*").eq("id", patientId).maybeSingle(),
      supabase.from("patient_notes").select("id, created_at, titulo, contenido").eq("patient_id", patientId).is("deleted_at", null).order("created_at", { ascending: false }),
      supabase.from("patient_appointments").select("cal_uid, title, status, start, end, meeting_url").eq("patient_id", patientId).order("start", { ascending: false }),
  ...
  const tagsList = (pts ?? []).map((r: any) => r.tags?.name).filter(Boolean).join(", ");
  ...
  for (const n of (notes ?? [])) {
    lines.push(["Nota", escape(n.id), escape(n.created_at), escape(n.titulo), escape(n.contenido)].join(","));
  }

  lines.push("");
  lines.push("Seccion,UID,Inicio,Fin,Titulo,Estado,URL");
  for (const a of (appts ?? [])) {
    lines.push(["Cita", escape(a.cal_uid), escape(a.start), escape(a.end), escape(a.title), escape(a.status), escape(a.meeting_url)].join(","));
  }
