...
const ListQuery = z.object({
  ...
  include_inactive: z.union([z.literal("true"), z.literal("false")]).optional().transform(v => v === "true"),
...
const UpsertBody = z.object({
  ...
  content: z.any().optional(), // JSON: { clauses: [{key,label,required,defaultChecked}], notes?:string }
  is_active: z.boolean().optional().default(true),
  provider_id: z.string().uuid().nullable().optional(), // null => plantilla de la org, o específica del proveedor
});
...
export async function POST(req: NextRequest) {
  ...
  const payload = {
    ...
    provider_id: p.data.provider_id ?? null,
    created_by: creator,
  };
  ...
  if (p.data.id) {
    ...
    const { data, error } = await supa
      .from("agreements_templates")
      .upsert(payload, { onConflict: "org_id,coalesce(provider_id,'00000000-0000-0000-0000-000000000000'),type,slug" })
      .select("*")
      .single();
    if (error) return jsonError("DB_ERROR", error.message, 400);
    return jsonOk(data);
