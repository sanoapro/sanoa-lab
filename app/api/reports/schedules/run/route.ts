...
async function sendViaReminders(org_id: string, channel: "whatsapp"|"sms", target: string, text: string) {
...
export async function POST(req: NextRequest) {
  ...
  for (const s of schedules ?? []) {
    ...
    const text = s.report === "daily_summary"
    ...
    try {
      ...
      // marca ejecución
      await svc.from("report_schedules").update({ last_run_at: nowUTC.toISOString() }).eq("id", s.id);
      executed.push(s.id);
    } catch {
