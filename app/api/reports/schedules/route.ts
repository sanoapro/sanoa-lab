...
function nowPartsTZ(tz: string) {
  ...
  const fmt = new Intl.DateTimeFormat("en-GB", {
    timeZone: tz, weekday: "short", hour: "2-digit", minute: "2-digit", hour12: false, year:"numeric", month:"2-digit", day:"2-digit"
  });
...
async function sendViaReminders(origin: string, org_id: string, channel: "whatsapp"|"sms", target: string, text: string) {
  await fetch(`${origin}/api/reminders/schedule`, {
    method:"POST",
    headers: { "content-type":"application/json" },
    body: JSON.stringify({
      org_id,
      item: { channel, target, template: "reporte_auto", schedule_at: new Date().toISOString(), meta: { message: text, source: "report_schedule" } }
    })
  }).catch(()=>{});
}
...
export async function POST(req: NextRequest) {
  ...
  for (const s of schedules ?? []) {
    ...
    try {
      ...
      if (s.report === "agenda_alerts") {
        ...
        if (scope === "org") {
          if (counts.total >= min_n && orgRate >= threshold) {
            ...
            msgs.push(`Alerta no-show (ORG): ${ratePct}% (${counts.ns}/${counts.total}) últimos ${window_days}d`);
          }
          ...
          for (const [key, r] of byRes) {
            ...
            if (rate >= threshold) {
              ...
              msgs.push(`Alerta no-show (Recurso ${r.name}): ${ratePct}% (${r.ns}/${r.total}) últimos ${window_days}d`);
              // registrar log por recurso+fecha
              await svc.from("agenda_alert_log").upsert({
                schedule_id: s.id, org_id: s.org_id, key, date_key: nowT.dateISO, sent_at: new Date().toISOString()
          ...
          for (const [key, r] of byPat) {
            ...
            if (rate >= threshold) {
              const ratePct = Math.round(rate*1000)/10;
              msgs.push(`Alerta no-show (Paciente ${r.name}): ${ratePct}% (${r.ns}/${r.total}) últimos ${window_days}d`);
              await svc.from("agenda_alert_log").upsert({
                schedule_id: s.id, org_id: s.org_id, key, date_key: nowT.dateISO, sent_at: new Date().toISOString()
              }, { onConflict: "schedule_id,key,date_key" });
            }
      ...
      await svc.from("report_schedules").update({ last_run_at: new Date().toISOString() }).eq("id", s.id);
      executed.push(s.id);
    } catch {
