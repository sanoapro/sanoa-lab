// app/api/reports/confirmations/summary/route.ts
...
export async function GET(req: Request){
  ...
  const start7 = daysAgoISO(6); // últimos 7 días incluyendo hoy

  const whereOrg = (q:any)=> org_id ? q.eq('org_id', org_id) : q;

  const { data: remToday } = await whereOrg(
    supa.from('reminders').select('status').gte('created_at', startToday)
  );
  const { data: logsToday } = await whereOrg(
    supa.from('reminder_logs').select('status').gte('created_at', startToday)
  );

  const { data: rem7 } = await whereOrg(
    supa.from('reminders').select('status, created_at').gte('created_at', start7)
  );
  const { data: logs7 } = await whereOrg(
    supa.from('reminder_logs').select('status, created_at').gte('created_at', start7)
  );

  const count = (arr:any[], field:string, val:string)=> (arr||[]).filter(x=>x[field]===val).length;
