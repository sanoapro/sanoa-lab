  ...
  try {
    ...
    if (letterheadPath) {
      const bytes = await fetchArrayBuffer(
        `${url.origin}/api/storage/letterheads/${letterheadPath}`,
      );
      if (bytes) {
        try {
          let img: any;
    ...
    page.drawText("Resumen de Agenda", { x: L, y, size: 18, font: bold });
    y -= 22;
    page.drawText(`Periodo: ${from} → ${to}   •   TZ: ${tz}`, { x: L, y, size: 11, font });
    y -= 18;
    ...
    for (const s of lines) {
      page.drawText(s, { x: L, y, size: 11, font });
      y -= 14;
    }
    ...
    function ensureSpace(rowH: any = 14) {
      if (y < 60) {
        ...
        p.drawText("Resumen de Agenda (cont.)", { x: L, y, size: 12, font: bold });
        y -= 18;
        // redraw header
        p.drawText(header[0], { x: colX[0], y, size: 11, font: bold });
        for (let i = 1; i < header.length; i++)
          p.drawText(header[i], { x: colX[i], y, size: 11, font: bold });
        y -= 14;
        return p;
      }
      return page;
