...
export async function GET(req: NextRequest) {
  ...
  if (!org_id || !fromA || !toA || !fromB || !toB) {
    return NextResponse.json({ ok:false, error:{ code:"BAD_REQUEST", message:"org_id, fromA, toA, fromB, toB requeridos" }}, { status:400 });
  }

  const qpA = new URLSearchParams({ org_id, from: fromA, to: toA });
  const qpB = new URLSearchParams({ org_id, from: fromB, to: toB });
  if (resource) { qpA.set("resource", resource); qpB.set("resource", resource); }
