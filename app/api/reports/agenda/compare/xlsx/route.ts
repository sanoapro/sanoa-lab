...
export async function GET(req: NextRequest) {
  ...
  const r = await fetch(`${url.origin}/api/reports/agenda/compare/json?${params.toString()}`, {
    cache: "no-store",
  });
  ...
  if (!j?.ok)
    return new Response(
      JSON.stringify({
        ok: false,
        error: { code: "UPSTREAM", message: "No se pudo calcular comparación" },
      }),
      { status: 500 },
    );
    ...
    XLSX.utils.json_to_sheet([
      {
        ...
        fromA,
        toA,
        totalA: A.totals.total,
        completedA: A.totals.completed,
        no_showA: A.totals.no_show,
        cancelA: A.totals.cancelled,
        durA_min: A.totals.avg_duration_min,
        leadA_h: A.totals.avg_lead_time_h,
        fromB,
        toB,
        totalB: B.totals.total,
        completedB: B.totals.completed,
        no_showB: B.totals.no_show,
        cancelB: B.totals.cancelled,
        durB_min: B.totals.avg_duration_min,
        leadB_h: B.totals.avg_lead_time_h,
      },
    ]),
    "Resumen",
  );
  ...
  const byDay = A.by_day
    .map((x: any) => ({ periodo: "A", ...x }))
    .concat(B.by_day.map((x: any) => ({ periodo: "B", ...x })));
  XLSX.utils.book_append_sheet(wb, XLSX.utils.json_to_sheet(byDay), "Por día");

  const byRes = A.by_resource
    .map((x: any) => ({ periodo: "A", ...x }))
    .concat(B.by_resource.map((x: any) => ({ periodo: "B", ...x })));
