...
export async function GET(req: NextRequest) {
  ...
  let q = supa
    .from("reminder_queue")
    .select("id, org_id, assignment_id, channel, template_slug, status, attempt_count, next_attempt_at, sent_at, last_error, created_at, payload")
  ...
  return jsonOk(data ?? [], { page: Math.floor(parsed.data.offset / parsed.data.limit) + 1, pageSize: parsed.data.limit, total: count ?? undefined });
}
