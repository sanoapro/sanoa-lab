...
export async function GET(req: NextRequest) {
  ...
  (targets || []).forEach((t: T) =>
    tmap.set(`${t.patient_id}:${t.type}`, { low: t.target_low, high: t.target_high }),
  );

  const perType = new Map<
    string,
    { total: number; in_range: number; latest?: { value: number; at: string | null } }
  >();
  (meas || []).forEach((m: M) => {
    const key = m.type;
  ...
  return NextResponse.json({
    ok: true,
    data,
    meta: { scope: patient_id ? "patient" : "org", from, to },
  });
}
