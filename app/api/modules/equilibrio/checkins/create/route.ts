...
export async function POST(req: NextRequest) {
  ...
  const body = await req.json().catch(()=>null) as {
    org_id?: string; patient_id?: string; plan_id?: string; item_id?: string;
    date?: string; status?: "done"|"skipped"; note?: string|null;
  };
  ...
  const yyyy_mm_dd = new Date(Date.UTC(day.getUTCFullYear(), day.getUTCMonth(), day.getUTCDate())).toISOString().slice(0,10);

  const { data, error } = await supa
    .from("equilibrio_checkins")
    .insert({
      org_id: body.org_id, patient_id: body.patient_id,
      plan_id: body.plan_id, item_id: body.item_id,
      day: yyyy_mm_dd, status: body.status,
