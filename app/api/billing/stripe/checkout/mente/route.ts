...
export async function POST(req: Request){
  try{
    ...
    const price = process.env.STRIPE_PRICE_MENTE_PRO!;
    if (!price) return NextResponse.json({ error: "Falta STRIPE_PRICE_MENTE_PRO" }, { status: 400 });
