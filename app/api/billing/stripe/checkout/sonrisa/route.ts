...
export async function POST(req: Request){
  try{
    ...
    const price = process.env.STRIPE_PRICE_SONRISA_PRO!;
    if (!price) return NextResponse.json({ error: "Falta STRIPE_PRICE_SONRISA_PRO" }, { status: 400 });
    ...
    const session = await stripe.checkout.sessions.create({
      ...
      metadata: { org_id, module: "sonrisa" },
    });
