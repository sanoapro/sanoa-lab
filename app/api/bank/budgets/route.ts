...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    const items: { category_id: string; month: string; amount_cents: number }[] | undefined = body?.items;
    ...
    const { data, error } = await supa.from("bank_budgets").upsert(payload, { onConflict: "org_id,category_id,month" }).select("*");
    if (error) return NextResponse.json({ ok: false, error: { code: "DB_ERROR", message: error.message } }, { status: 400 });
