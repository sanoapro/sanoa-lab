...
export async function DELETE(req: NextRequest, { params }: { params: { id: string } }) {
  ...
  try {
    ...
    const id = params.id;
    if (!id) return NextResponse.json({ ok: false, error: { code: "BAD_REQUEST", message: "Falta id" } }, { status: 400 });

    const { data, error } = await supa.from("bank_rules").delete().eq("org_id", org_id).eq("id", id).select("*");
