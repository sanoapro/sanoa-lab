...
export async function PATCH(req: NextRequest) {
  ...
  try {
    ...
    if (!org_id || !Array.isArray(ids) || ids.length === 0 || !set) {
      return NextResponse.json({ ok: false, error: { code: "BAD_REQUEST", message: "org_id, ids y set requeridos." } }, { status: 400 });
    }

    const allowed: Partial<Tx> = {};
    if (set.status && ["pending", "cleared"].includes(set.status)) allowed.status = set.status as Tx["status"];
    ...
    if (Object.keys(allowed).length === 0) {
      return NextResponse.json({ ok: false, error: { code: "BAD_REQUEST", message: "Nada que actualizar." } }, { status: 400 });
    }
