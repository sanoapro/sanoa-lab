...
export async function GET(_: Request, { params }: { params: { id: string } }) {
  ...
  const { data: items } = await supabase.from("lab_request_items").select("*").eq("request_id", id).order("id");
  const { data: pat } = await supabase.from("patients").select("full_name, external_id").eq("id", req.patient_id).maybeSingle();
  const { data: lh } = await supabase.from("doctor_letterheads").select("*").eq("org_id", req.org_id).eq("doctor_id", req.requested_by).maybeSingle();

  let footer = lh?.footer_disclaimer || "";
  if (!footer) { const { data: d } = await supabase.from("org_disclaimers").select("text").eq("org_id", req.org_id).eq("kind","labs").maybeSingle(); footer = d?.text || footer; }

  const { data: led } = await supabase.rpc("ensure_document_folio", { p_doc_type: "lab_request", p_doc_id: id });
  ...
  // Título y paciente
  y -= 8; page.drawText("Solicitud de estudios de laboratorio", { x: 40, y, size: 12 }); y -= 14;
  page.drawText(`Paciente: ${pat?.full_name || req.patient_id}`, { x: 40, y, size: 10 }); y -= 12;
  if (pat?.external_id) { page.drawText(`ID: ${pat.external_id}`, { x: 40, y, size: 10 }); y -= 12; }
  if (req.title)        { y = drawWrappedText(page, `Título: ${req.title}`, 40, y-6, W-80, 10); }
  if (req.instructions) { y = drawWrappedText(page, `Instrucciones: ${req.instructions}`, 40, y-6, W-80, 10); }
  if (req.due_at)       { page.drawText(`Fecha límite: ${new Date(req.due_at).toLocaleString()}`, { x: 40, y: y-6, size: 10 }); y -= 18; }

  // Ítems solicitados
  page.drawText("Estudios solicitados:", { x: 40, y, size: 12 }); y -= 14;
  ...
  if (lh?.signature_url) {
    ...
    if (ab) {
      const img = await pdf.embedPng(ab);
      const w = 140, h = (img.height / img.width) * w;
  ...
  if (verifyUrl.length > 46) page.drawText(verifyUrl.slice(46, 92), { x: W - 160, y: 122, size: 8 });
