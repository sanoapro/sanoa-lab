...
export async function POST(req: NextRequest) {
  ...
  // Intento de RPC si existe (nombre ilustrativo)
  ...
  // Fallback si el RPC no existe (no rompe el flujo de UX)
  if (error && (error.code === "PGRST204" || /function.*does not exist/i.test(error.message))) {
