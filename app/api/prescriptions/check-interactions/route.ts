...
export async function POST(req: Request) {
  ...
  if (!patient_id || !Array.isArray(items)) return NextResponse.json({ error: "bad_request" }, { status: 400 });

  // Condiciones y medicamentos actuales del paciente
  const [condsRes, medsRes] = await Promise.all([
    supabase.from("patient_conditions").select("concept:concept_dictionary(id, canonical, canonical_norm)").eq("patient_id", patient_id),
  ...
  const names = Array.from(new Set([...currentMeds, ...items.map(i => i.drug)]
  ...
  if (proposedIngr.length && currentIngr.length) {
    ...
    for (const a of proposedIngr) for (const b of currentIngr) {
