...
export async function GET(_: Request, { params }: { params: { id: string } }) {
  ...
  // Folio + verificación
  const { data: led } = await supabase.rpc("ensure_document_folio", { p_doc_type: "prescription", p_doc_id: id });
  ...
  // Paciente
  y -= 8; page.drawText("Paciente:", { x: 40, y, size: 12 }); y -= 14;
  page.drawText(`${pat?.full_name || rx.patient_id}`, { x: 40, y, size: 10 }); y -= 12;
  if (pat?.external_id) { page.drawText(`ID: ${pat.external_id}`, { x: 40, y, size: 10 }); y -= 12; }

  // Diagnóstico
  if (rx.diagnosis) {
    y -= 6; page.drawText("Diagnóstico:", { x: 40, y, size: 12 }); y -= 14;
    y = drawWrappedText(page, rx.diagnosis, 40, y, W - 80, 10);
  }

  // Items
  y -= 6; page.drawText("Prescripción:", { x: 40, y, size: 12 }); y -= 14;
