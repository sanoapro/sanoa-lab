...
export default function ImportPage() {
  ...
  async function doImport() {
    ...
    try {
      ...
      showToast({ title: "Importación completa", description: `${rows.length} pacientes creados.` });
    } catch (e: any) {
  ...
  return (
    <div className="max-w-3xl mx-auto p-4 space-y-4">
      ...
      <p className="text-sm text-gray-600">Pega aquí un CSV con columnas <code>nombre, edad, genero</code>. Cada fila crea un paciente. Se asignará a tu <strong>organización activa</strong> si existe.</p>
      <textarea className="w-full h-64 border rounded-md p-3 font-mono text-sm" value={text} onChange={(e) => setText(e.target.value)} />
      <div className="flex items-center justify-between">
        <div className="text-sm text-gray-600">Creados: {done}</div>
        <Button onClick={() => void doImport()} disabled={importing}>Importar</Button>
      </div>
    </div>
  );
}
