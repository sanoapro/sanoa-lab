...
function SessionsList({ orgId, patientId }: { orgId: string; patientId: string }) {
  ...
  async function load() {
    ...
    const r = await fetch(`/api/modules/mente/sessions/list?${p.toString()}`, { cache: "no-store" });
  ...
  return (
    <div className="border rounded-2xl p-4 space-y-3">
      <div className="flex items-center justify-between">
        ...
        <button className="border rounded px-3 py-2" onClick={load}>Actualizar</button>
      </div>
      <div className="rounded border overflow-auto">
        <table className="w-full text-sm">
          <thead className="bg-gray-50">
            <tr><th className="text-left px-3 py-2">Fecha</th><th className="text-left px-3 py-2">Firmada</th></tr>
          </thead>
          <tbody>
            {rows.map((r)=>(
            ...
            {!rows.length && <tr><td colSpan={2} className="px-3 py-6 text-center">Sin notas</td></tr>}
          </tbody>
        </table>
      </div>
    </div>
    ...
    <main className="p-6 md:p-10 space-y-6">
      ...
      {!orgId ? (
        ...
        <>
          ...
          {patient && (
            <>
              <SimpleNote orgId={orgId} patientId={patient.id} onSaved={() => { /* opcional: toasts */ }} />
