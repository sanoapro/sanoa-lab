    ...
    <main className="p-6 md:p-10 space-y-8">
      ...
      <section className={box}>
        ...
        <div className="grid md:grid-cols-6 gap-3 mt-4">
          <L label="Peso (kg)">
            <I value={peso} onChange={setPeso} />
          </L>
          <L label="Talla (cm)">
            <I value={talla} onChange={setTalla} />
          </L>
          <L label="Edad (años)">
            <I value={edad} onChange={setEdad} />
          </L>
          <L label="Sexo">
            <select
              value={sexo}
          ...
          <L label="Creatinina (mg/dL)">
            <I value={creat} onChange={setCreat} />
          </L>
      ...
      <section className="grid md:grid-cols-2 gap-6">
        <div className={box}>
          ...
          <div className="grid md:grid-cols-4 gap-3 mt-4">
            <L label="Na (mEq/L)">
              <I value={Na} onChange={setNa} />
            </L>
            <L label="Cl (mEq/L)">
              <I value={Cl} onChange={setCl} />
            </L>
            <L label="HCO₃⁻ (mEq/L)">
              <I value={HCO3} onChange={setHCO3} />
            </L>
            <div className="rounded-xl border p-3">
              <div className="text-xs text-[var(--color-brand-bluegray)]">Resultado</div>
              <div className="text-lg font-semibold">
                {isNaN(anionGap) ? "—" : anionGap.toFixed(0)} mEq/L
              </div>
            </div>
          </div>
        </div>
        ...
        <div className={box}>
          ...
          <div className="grid md:grid-cols-3 gap-3 mt-4">
            <L label="QT (ms)">
              <I value={qt} onChange={setQt} />
            </L>
            <L label="RR (ms)">
              <I value={rr} onChange={setRr} />
            </L>
            <div className="rounded-xl border p-3">
              <div className="text-xs text-[var(--color-brand-bluegray)]">Resultado</div>
