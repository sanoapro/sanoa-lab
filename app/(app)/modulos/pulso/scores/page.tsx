...
function calcCURB({confusion, ureaHigh, rrHigh, hypotension, age65}:{confusion:boolean; ureaHigh:boolean; rrHigh:boolean; hypotension:boolean; age65:boolean;}){
  return [confusion, ureaHigh, rrHigh, hypotension, age65].filter(Boolean).length;
}

/** CHA2DS2-VASc (AFib) */
function calcCHA2DS2VASC({ccf, htn, age75, dm, strokeTIA, vasc, age65_74, female}:{
  ccf:boolean; htn:boolean; age75:boolean; dm:boolean; strokeTIA:boolean; vasc:boolean; age65_74:boolean; female:boolean;
}){
  let s = 0;
  s += ccf ? 1 : 0;
  s += htn ? 1 : 0;
...
/** HAS-BLED */
function calcHASBLED({htn, renal, liver, stroke, bleed, inrLab, age65, drugsAlcohol}:{
  htn:boolean; renal:boolean; liver:boolean; stroke:boolean; bleed:boolean; inrLab:boolean; age65:boolean; drugsAlcohol:boolean;
}){
  let s = 0;
...
function CURB65(){
  ...
  const risk = s>=3 ? "Alta mortalidad / valorar hospitalización" : s===2 ? "Riesgo intermedio" : "Riesgo bajo";
  return (
...
function CHA2DS2VASC(){
  const [st,set] = useState({ccf:false, htn:false, age75:false, dm:false, strokeTIA:false, vasc:false, age65_74:false, female:false});
...
function HASBLED(){
  const [st,set] = useState({htn:false, renal:false, liver:false, stroke:false, bleed:false, inrLab:false, age65:false, drugsAlcohol:false});
  ...
  return (
    <section className={box}>
      ...
      <div className="grid md:grid-cols-4 gap-3 mt-4">
        ...
        <Chk label="Función hepática deficiente" v={st.liver} set={(v)=>set({...st, liver:v})}/>
        ...
        <Chk label="Drogas/Alcohol" v={st.drugsAlcohol} set={(v)=>set({...st, drugsAlcohol:v})}/>
      </div>
      <ScoreDisplay value={s} note={risk}/>
    </section>
  );
...
function QSOFA(){
  ...
  const s = useMemo(()=> calcQSOFA({ fr:num(fr), pas:num(pas), glasgow:num(gcs) }), [fr,pas,gcs]);
  ...
  return (
    <section className={box}>
      ...
      <div className="grid md:grid-cols-3 gap-3 mt-4">
        ...
        <L label="Glasgow"><I v={gcs} set={setG} /></L>
      </div>
      <ScoreDisplay value={s} note={risk}/>
    </section>
  );
