...
export default function SessionsList({ params }:{ params:{ id:string } }){
  ...
  return (
    <div className="p-6 space-y-4">
      ...
      <div>
        <Link href={`/modulos/equilibrio/pacientes/${params.id}/sesiones/new`} className="underline">+ Nueva sesión</Link>
      </div>
              ...
              {items.map(it=>(
                <li key={it.id} className="flex items-center justify-between border rounded p-2">
                  <div className="text-sm">
                    ...
                    <div className="opacity-60 text-xs">Creado: {new Date(it.created_at).toLocaleString()}</div>
                  </div>
                  <div className="flex gap-3 text-sm">
                    <Link className="underline" href={`/modulos/equilibrio/sesiones/${it.id}`}>Abrir</Link>
                    <a className="underline" href={`/print/equilibrio/sesiones/${it.id}`} target="_blank" rel="noreferrer">Imprimir</a>
                  </div>
                </li>
              ))}
            </ul>
