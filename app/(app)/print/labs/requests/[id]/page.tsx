  ...
  return (
    <div className="p-8 print:p-8 max-w-3xl mx-auto">
      ...
      <div className="text-sm">
        <div><b>Paciente:</b> {patient?.full_name || request.patient_id}</div>
        {patient?.external_id && <div><b>ID:</b> {patient.external_id}</div>}
      </div>

      <h2 className="mt-4 font-medium">Solicitud de estudios de laboratorio</h2>
      {request.title && <div className="mt-1 text-sm"><b>Título:</b> {request.title}</div>}
      {request.due_at && <div className="text-sm"><b>Fecha límite:</b> {new Date(request.due_at).toLocaleString()}</div>}
      {request.instructions && <div className="mt-2 whitespace-pre-line text-sm"><b>Instrucciones:</b> {request.instructions}</div>}

      <h3 className="mt-4 font-medium">Estudios</h3>
      <ul className="list-disc pl-6 text-sm">
        {(items||[]).map((it:any)=>(
          <li key={it.id} className="mb-1">
            <div><b>{it.test_name || "(sin nombre)"}</b>{it.test_code ? ` [${it.test_code}]` : ""}</div>
      ...
      <div className="mt-8 flex items-center justify-between">
        ...
        {letterhead?.signature_url && (
          <img src={`/api/storage/signatures/${letterhead.signature_url}`} alt="firma" className="h-20" />
        )}
      </div>
