  ...
  return (
    <div className="p-8 print:p-8 max-w-3xl mx-auto">
      ...
      <div className="text-sm">
        <div>
          <b>Paciente:</b> {patient?.full_name || discharge.patient_id}
        </div>
      </div>
      <h2 className="mt-4 font-medium">Resumen de alta</h2>
      <div className="space-y-2 text-sm">
        {discharge.admission_at && (
          <div>
            <b>Ingreso:</b> {new Date(discharge.admission_at).toLocaleString()}
          </div>
        )}
        {discharge.discharge_at && (
          <div>
            <b>Alta:</b> {new Date(discharge.discharge_at).toLocaleString()}
          </div>
        )}
        {discharge.diagnosis && (
          <div>
            <b>Diagnóstico:</b> <span className="whitespace-pre-line">{discharge.diagnosis}</span>
          </div>
        )}
        {discharge.summary && (
          <div>
            <b>Resumen:</b> <span className="whitespace-pre-line">{discharge.summary}</span>
          </div>
        )}
        {discharge.recommendations && (
          <div>
            <b>Recomendaciones:</b>{" "}
            <span className="whitespace-pre-line">{discharge.recommendations}</span>
          </div>
        )}
        {discharge.follow_up_at && (
          <div>
            <b>Seguimiento:</b> {new Date(discharge.follow_up_at).toLocaleString()}
          </div>
        )}
      </div>
