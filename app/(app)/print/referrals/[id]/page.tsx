...
export default function PrintReferral({ params }: { params: { id: string } }) {
  ...
  useEffect(()=>{(async()=>{ const j = await fetch(`/api/referrals/${id}/json`).then(r=>r.json()); setData(j); setTimeout(()=>window.print(), 500); })();},[id]);
  ...
  return (
    <div className="p-8 print:p-8 max-w-3xl mx-auto">
      ...
      <div className="text-sm">
        <div><b>Paciente:</b> {patient?.full_name || referral.patient_id}</div>
      </div>
      <h2 className="mt-4 font-medium">Interconsulta</h2>
      <div className="space-y-2 text-sm">
        {referral.to_specialty && <div><b>Especialidad:</b> {referral.to_specialty}</div>}
        {referral.to_doctor_name && <div><b>Dirigida a:</b> {referral.to_doctor_name}</div>}
        {referral.reason && <div><b>Motivo:</b> <span className="whitespace-pre-line">{referral.reason}</span></div>}
        {referral.summary && <div><b>Resumen:</b> <span className="whitespace-pre-line">{referral.summary}</span></div>}
        {referral.plan && <div><b>Plan sugerido:</b> <span className="whitespace-pre-line">{referral.plan}</span></div>}
      </div>
