...
export default function OrgsPage() {
  ...
  async function load() {
    ...
    for (const o of list) {
      try {
        obj[o.id] = await listMembers(o.id);
      } catch {
        obj[o.id] = [];
      }
    }
  ...
  async function create() {
    if (!name.trim()) {
      showToast({ title: "Valida", description: "Nombre requerido." });
      return;
    }
  ...
  return (
    <div className="max-w-4xl mx-auto p-4 space-y-6">
          ...
          <div key={o.id} className="p-4 space-y-2">
            <div className="flex items-center justify-between">
              <div className="font-medium">
                {o.name} <span className="text-xs text-gray-500">({o.id.slice(0, 8)}…)</span>
              </div>
              <div className="flex gap-2">
                <Button
                  variant={active.id === o.id ? "secondary" : "default"}
                  onClick={() => activate(o)}
                >
            ...
            <div className="rounded-md border">
              {(members[o.id] ?? []).map((m) => (
                ...
                >
                  <div className="text-sm">
                    {m.user_id} · rol: <strong>{m.role}</strong>
                  </div>
                  <div className="flex gap-2">
                    <Button
                      variant="secondary"
                      onClick={() => void updateRole(o.id, m.user_id, "member")}
                    >
                      Member
                    </Button>
                    <Button
                      variant="secondary"
                      onClick={() => void updateRole(o.id, m.user_id, "admin")}
                    >
                      Admin
                    </Button>
                  </div>
              ...
              {(members[o.id] ?? []).length === 0 && (
                <div className="p-2 text-sm text-gray-600">Sin miembros visibles.</div>
              )}
            </div>
          </div>
        ))}
      ...
      <p className="text-xs text-gray-500">
        Tip: La “organización activa” se usa para asignar org_id al crear pacientes y para filtrar
        la lista cuando elijas esa opción.
      </p>
    </div>
  );
}
