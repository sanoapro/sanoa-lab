...
import { listExportTemplates, createExportTemplate, deleteExportTemplate, type ExportTemplate } from "@/lib/export-templates";
...
export default function ExportAdvancedPage() {
  ...
  async function createTpl() {
    if (!tplName.trim()) { showToast({ title: "Nombre requerido", description: "Escribe un nombre para la plantilla." }); return; }
    try {
      const t = await createExportTemplate(tplName.trim(), tplTitle.trim(), tplSubtitle || null, tplLogo || null, tplColor || null, tplScope);
      setTemplates([t, ...templates]); setTplId(t.id);
      setTplName("");
  ...
  function Cover() {
    const t = activeTpl();
    if (!t) return null;
    const color = (t.brand_hex || "#D97A66").startsWith("#") ? (t.brand_hex || "#D97A66") : `#${t.brand_hex}`;
    return (
      <section className="h-[70vh] min-h-[520px] flex flex-col items-center justify-center text-center relative">
        <div className="absolute inset-0" style={{ background: `linear-gradient(135deg, ${color}22, transparent)` }} />
        {t.logo_url && <img src={t.logo_url} alt="Logo" className="h-16 mb-4" />}
        <h1 className="text-3xl font-semibold" style={{ color }}>{t.cover_title}</h1>
        <p className="mt-2 text-gray-700">{t.cover_subtitle ?? ""}</p>
        <div className="mt-8 text-sm text-gray-600">
          <div><strong>Paciente:</strong> {patient?.nombre}</div>
  ...
  return (
    <div className="max-w-4xl mx-auto p-4 space-y-4">
      ...
      <div className="border rounded-xl p-4 bg-white grid grid-cols-1 sm:grid-cols-3 gap-3">
        ...
        <label className="text-sm flex items-center gap-2"><input type="checkbox" checked={incFiles} onChange={(e)=>setIncFiles(e.target.checked)} /> Archivos (listado)</label>
      ...
      <div className="border rounded-xl p-4 bg-white space-y-3">
        ...
        <div className="flex gap-2 items-center">
          <select className="border rounded-md px-3 py-2" value={tplId} onChange={(e)=>setTplId(e.target.value)}>
        ...
        <details className="rounded border p-3 bg-gray-50">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 mt-3">
            ...
            <select className="border rounded-md px-3 py-2" value={tplScope} onChange={(e)=>setTplScope(e.target.value as any)}>
              <option value="personal">Personal</option>
              <option value="org">Organización activa</option>
            </select>
            <Input placeholder="Título de portada" value={tplTitle} onChange={(e)=>setTplTitle(e.target.value)} />
            <Input placeholder="Subtítulo (opcional)" value={tplSubtitle} onChange={(e)=>setTplSubtitle(e.target.value)} />
            <Input placeholder="Logo URL (opcional)" value={tplLogo} onChange={(e)=>setTplLogo(e.target.value)} />
            <Input placeholder="Color marca (HEX)" value={tplColor} onChange={(e)=>setTplColor(e.target.value)} />
          </div>
      ...
      <div className="flex justify-end">
        <ExportPDFButton targetRef={targetRef} fileName={`Export-${(patient?.nombre || "Paciente").replace(/\s+/g,"_")}-${new Date().toISOString().slice(0,16).replace(/[:T]/g,"")}.pdf`} />
      </div>
    </div>
  );
}