...
export default function LabRequestDetailPage() {
  ...
  useEffect(() => { void load(); /* eslint-disable-next-line */ }, [requestId]);
  ...
  async function runOCR(res: Result) {
    setOcrBusy(res.id); setOcrText(null);
    try {
      if (!res.mime_type || !res.mime_type.startsWith("image/")) {
        showToast({ title: "OCR", description: "El OCR rápido funciona para imágenes (PNG/JPG). Para PDF lo veremos luego.", variant: "secondary" });
        return;
      }
      const url = await getSignedUrl(res.file_path);
      const blob = await fetch(url).then(r=>r.blob());
      const { data: { text } } = await Tesseract.recognize(await blob.arrayBuffer(), "spa");
  ...
  return (
    <div className="max-w-4xl mx-auto p-4 space-y-4">
      ...
      <div className="border rounded-xl bg-white dark:bg-slate-900 divide-y">
        {items.length === 0 && <div className="p-4 text-sm text-slate-600 dark:text-slate-300">{busy ? "Cargando…" : "Sin resultados."}</div>}
        {items.map((r) => (
            ...
            <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-2">
              <div className="min-w-0">
                ...
                <div className="text-xs text-slate-600 dark:text-slate-400">
                  {new Date(r.created_at).toLocaleString()} · {r.size_bytes ? `${(r.size_bytes/1024).toFixed(1)} KB` : ""} · {r.reviewed_at ? "Revisado" : "Pendiente"}
                </div>
              </div>
              <div className="flex flex-wrap gap-2">
                <Button
                  variant="secondary"
                  onClick={async () => { const u = await getSignedUrl(r.file_path); window.open(u, "_blank"); }}
                ...
                <Button
                  ...
                  onClick={async () => {
                    ...
                    a.href = u; a.download = r.file_name; a.click();
                  }}
                  className="px-3 py-1.5"
                >
                  Descargar
                </Button>
                <Button variant="secondary" onClick={() => void runOCR(r)} disabled={!!ocrBusy} className="px-3 py-1.5">
                  {ocrBusy === r.id ? "OCR…" : "OCR rápido"}
                </Button>
                <Button onClick={() => void review(r.id)} disabled={!!r.reviewed_at} className="px-3 py-1.5">
                  Marcar revisado
                </Button>
              </div>
            </div>
