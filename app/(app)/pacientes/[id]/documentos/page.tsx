...
export default function PatientDocs({ params }: { params: { id: string } }) {
  ...
  const ensureFolio = async (t: string, id: string) => {
    const r = await fetch("/api/docs/ensure-folio", {
      method: "POST",
      ...
      <div className="border rounded">
        ...
        {filtered.map((it: any) => (
          ...
          >
            ...
            <div>
              {it.folio || (
                <button className="text-xs underline" onClick={() => ensureFolio(it.type, it.id)}>
                  Generar
                </button>
              )}
            </div>
            ...
            <div className="flex gap-2">
              <a
                className="underline"
                href={`/pacientes/${patientId}/documentos/${it.type}/${it.id}`}
              >
                Ver
              </a>
              {pdfHref(it.type, it.id) !== "#" && (
                <a
                  className="underline"
                  href={pdfHref(it.type, it.id)}
                  target="_blank"
                  rel="noreferrer"
                >
                  PDF
              ...
              {printHref(it.type, it.id) && (
                <a
                  className="underline"
                  href={printHref(it.type, it.id)}
                  target="_blank"
                  rel="noreferrer"
                >
                  Imprimir
            ...
            <div>
              {it.verify_code ? (
                <button
                  ...
                  onClick={() => navigator.clipboard.writeText(verifyUrl(it))}
                >
                  Copiar link
                </button>
              ) : (
