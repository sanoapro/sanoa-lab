...
export default function RxEditor({ params }: { params: { id: string } }) {
  ...
  const checkInteractions = async () => {
    ...
    const j = await r.json();
    alert(
      `Interacciones — Droga-Droga: ${j.warnings?.drug_drug?.length || 0} • Droga-Condición: ${j.warnings?.drug_condition?.length || 0}`,
    );
  };
  ...
  return (
    <div className="p-4 max-w-3xl space-y-3">
        ...
        <div className="border rounded p-2 max-h-48 overflow-auto text-sm">
          {results.map((r: any) => (
            <div key={r.id} className="flex justify-between items-center py-1">
              ...
              <button className="text-xs underline" onClick={() => addDrug(r.name)}>
                Añadir
              </button>
            </div>
          ))}
      ...
      <div className="flex gap-2 items-end">
        ...
        <button className="px-3 py-2 border rounded" onClick={applyTemplate}>
          Cargar plantilla
        </button>
      </div>
      ...
      <div className="flex gap-2">
        <button className="px-3 py-2 border rounded" onClick={checkInteractions}>
          Checar interacciones
        </button>
        <button className="px-3 py-2 bg-black text-white rounded" onClick={saveAndPdf}>
          Guardar + PDF
        </button>
      </div>
    </div>
