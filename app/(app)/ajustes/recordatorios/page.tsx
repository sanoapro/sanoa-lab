...
export default function RecordatoriosSettings() {
  ...
  const [form, setForm] = useState<Template>({
    org_id: "",
    name: "Recordatorio estándar",
    channel: "whatsapp",
    body: "{org_name}: Te esperamos el {date} a las {time}",
    active: true,
  });
    ...
    <div className="p-6 space-y-6 max-w-3xl">
        ...
        <div className="grid md:grid-cols-2 gap-4">
          <div className="space-y-2">
            ...
            <div className="flex gap-2">
              ...
              <button onClick={sendTest} className="px-3 py-2 rounded border">
                Enviar prueba
              </button>
            </div>
          </div>
              ...
              {list.map((t: any) => (
                <li key={t.id} className="border rounded p-2">
                  ...
                  <div className="text-xs opacity-70">
                    Canal: {t.channel} · Activa: {t.active ? "sí" : "no"}
                  </div>
                  <pre className="text-xs bg-black/5 p-2 rounded mt-1">{t.body}</pre>
                  <button className="text-xs underline mt-1" onClick={() => setForm(t)}>
                    Editar
                  </button>
                </li>
              ))}
      ...
      <section className="space-y-2">
        ...
        <p className="text-sm opacity-80">
          Programa un CRON cada 5 minutos a <code>/api/jobs/reminders/run</code>. Programa el
          resumen diario (ej. 20:00) a <code>/api/reports/daily-summary/send</code> con{" "}
          <code>{"{ org_id, to }"}</code>.
        </p>
      </section>
    </div>
