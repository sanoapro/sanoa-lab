...
export default function VerifyPage() {
  ...
  useEffect(()=>{
    (async ()=>{
      ...
      if(!type || !id || !code){ setError("Parámetros incompletos"); return; }
      const j = await fetch(`/api/docs/verify?type=${encodeURIComponent(type)}&id=${encodeURIComponent(id)}&code=${encodeURIComponent(code)}`).then(r=>r.json());
      setInfo(j);
    })();
  },[]);
  ...
  return (
    <div className="p-6 max-w-xl mx-auto space-y-2">
      ...
      <div>Folio: <b>{info.folio || "—"}</b></div>
      <div>Emitido: {info.created_at ? new Date(info.created_at).toLocaleString() : "—"}</div>
      {info.revoked && <div>Revocado: {info.revoked_at ? new Date(info.revoked_at).toLocaleString() : "—"}</div>}
      <div className="text-xs text-gray-500 mt-2">Este comprobante refleja el estado en tiempo real.</div>
    </div>
  );
}
