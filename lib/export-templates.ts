...
export async function listExportTemplates(includeOrg = true): Promise<ExportTemplate[]> {
  ...
  if (includeOrg && active.id) {
    const { data, error } = await q.or(`owner_id.eq.${(await supabase.auth.getUser()).data.user?.id},org_id.eq.${active.id}`);
...
export async function createExportTemplate(
  ...
  const { data, error } = await supabase.from("export_templates").insert({
    ...
    name, cover_title, cover_subtitle, logo_url, brand_hex
  }).select("*").single();
  if (error) throw error;
  return data as ExportTemplate;
}
