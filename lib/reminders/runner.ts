...
export async function runReminderBatch(orgId?: string, limit = 20) {
  ...
  for (const r of due || []) {
    try {
      ...
      let body = '';
      if (r.template_id) {
        const { data: tpl } = await supa.from('reminder_templates').select('*').eq('id', r.template_id).maybeSingle();
        body = renderTemplate(tpl?.body || '', r.payload || {});
      } else {
        body = renderTemplate('{org_name}: Recordatorio de cita el {date} a las {time}', r.payload || {});
      }

      const channel = r.channel as 'sms'|'whatsapp';
      let sendRes: any;
      ...
      await supa.from('reminder_logs').insert({
        reminder_id: r.id, status: 'sent', provider: 'twilio', provider_sid: sendRes.sid, meta: { channel }
      });
      ...
      await supa.from('reminders').update({
        ...
        next_run_at: backoff(r.attempts + 1),
      }).eq('id', r.id);

      track('Reminder Sent', {
        org_id: r.org_id, channel, provider_sid: sendRes.sid, template_id: r.template_id ? 'tpl' : 'inline'
      });
      ...
      await supa.from('reminder_logs').insert({
        reminder_id: r.id, status: 'failed', provider: 'twilio', error: msg
      });

      const next = r.attempts + 1 < r.max_attempts ? 'retry' : 'failed';
      await supa.from('reminders').update({
        status: next,
        attempts: r.attempts + 1,
        last_attempt_at: new Date().toISOString(),
      ...
      track('Reminder Failed', {
        org_id: r.org_id, channel: r.channel, error: msg, template_id: r.template_id ? 'tpl' : 'inline'
      });
