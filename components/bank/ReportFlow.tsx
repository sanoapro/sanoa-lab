...
export default function ReportFlow({ orgId, from, to }: { orgId: string; from: string; to: string }) {
  ...
  const width = 480, height = 180, pad = 24;
        ...
        <tbody>
          {rows.map(r => (
            <tr key={r.month} className="border-t">
              ...
              <td className={`px-3 py-2 text-right ${r.net_cents >= 0 ? "text-emerald-700" : "text-rose-700"}`}>
