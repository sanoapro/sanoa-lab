...
export default function AssessmentsTimeline({ orgId, patientId }: { orgId: string; patientId: string }) {
  ...
  async function load() {
    ...
    const r = await fetch(`/api/modules/mente/assessments/list?${p.toString()}`, { cache: "no-store" });
    ...
    <div className="rounded-2xl border overflow-auto">
      <table className="w-full text-sm">
        ...
        <tbody>
          ...
          {!loading && rows.length === 0 && <tr><td colSpan={4} className="px-3 py-6 text-center">Sin evaluaciones</td></tr>}
          {rows.map((r) => (
            <tr key={r.id} className="border-t">
              <td className="px-3 py-2">{r.issued_at || new Date(r.created_at).toLocaleString()}</td>
              <td className="px-3 py-2 uppercase">{r.tool}</td>
              <td className="px-3 py-2">{r.score_total}</td>
              <td className="px-3 py-2">
                <span className={`inline-flex items-center px-2 py-0.5 rounded-full text-xs ${bandCls(r.risk_band)}`}>● {r.risk_band}</span>
              </td>
            </tr>
          ))}
        </tbody>
