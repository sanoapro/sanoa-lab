...
export default function AssignmentsTable() {
  ...
  React.useEffect(() => { load(); /* eslint-disable-next-line */ }, [org.id, status, module]);
  ...
  return (
    <div className="rounded-2xl border bg-white/95 dark:bg-slate-900/60">
      <div className="p-4 flex flex-wrap items-center gap-2">
        ...
        <select value={module} onChange={e => setModule(e.target.value)} className="rounded-lg border px-2 py-1 bg-white dark:bg-slate-900">
        ...
        <select value={status} onChange={e => setStatus(e.target.value)} className="rounded-lg border px-2 py-1 bg-white dark:bg-slate-900">
          <option value="">Todos</option>
          <option value="active">active</option>
          <option value="completed">completed</option>
          <option value="canceled">canceled</option>
        </select>
        <button onClick={() => bulk({ status: "completed", last_done_at: new Date().toISOString() })} className="px-3 py-1 rounded-lg border">Marcar hecho</button>
        <button onClick={() => bulk({ status: "canceled" })} className="px-3 py-1 rounded-lg border">Cancelar</button>
        ...
        <table className="w-full text-sm">
          <thead>
            <tr className="border-t border-b bg-slate-50/60 dark:bg-slate-800/40">
              <th className="p-2"><input type="checkbox" onChange={e => setSel(e.target.checked ? rows.map(r => r.id) : [])} checked={sel.length === rows.length && rows.length>0} /></th>
          ...
          <tbody>
            {rows.map(r => (
              <tr key={r.id} className="border-b">
                <td className="p-2"><input type="checkbox" checked={sel.includes(r.id)} onChange={() => toggle(r.id)} /></td>
            ...
            {rows.length === 0 && !loading && (
              <tr><td className="p-4 text-slate-500" colSpan={7}>Sin asignaciones.</td></tr>
            )}
          </tbody>
        </table>
      </div>
