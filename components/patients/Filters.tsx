...
export default function PatientsFilters() {
  ...
  const [includeDeleted, setIncludeDeleted] = useState((initial.includeDeleted ?? "false") === "true");

  function apply() {
    const p = new URLSearchParams(initial as any);
    function setOrDel(k: string, v: string) {
      if (v && v.trim()) p.set(k, v.trim()); else p.delete(k);
    }
    setOrDel("q", q);
    ...
    <div className="rounded-2xl border p-4 space-y-3">
      <div className="grid grid-cols-1 md:grid-cols-6 gap-3">
        <div className="md:col-span-2">
          ...
          <input className="rounded border px-3 py-2 w-full" value={q} onChange={e => setQ(e.target.value)} placeholder="Nombre o término…" />
        </div>
        <div>
          <label className="block text-sm mb-1">Género</label>
          <select className="rounded border px-3 py-2 w-full" value={genero} onChange={e => setGenero(e.target.value)}>
        ...
        <div className="flex items-end">
          <label className="inline-flex items-center gap-2">
            <input type="checkbox" className="rounded border" checked={includeDeleted} onChange={e => setIncludeDeleted(e.target.checked)} />
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-3">
        <div>
          ...
          <input className="rounded border px-3 py-2 w-full" value={tagsAny} onChange={e => setTagsAny(e.target.value)} placeholder="ej. DM2,HTA" />
        </div>
        <div>
          <label className="block text-sm mb-1">Tags (todas)</label>
          <input className="rounded border px-3 py-2 w-full" value={tagsAll} onChange={e => setTagsAll(e.target.value)} placeholder="ej. embarazo,alto_riesgo" />
        </div>
        <div className="flex items-end gap-2">
          <button className="rounded px-4 py-2 border" onClick={apply}>Aplicar</button>
          <button className="rounded px-4 py-2 border" onClick={reset}>Reset</button>
        </div>
      </div>
    </div>
  );
