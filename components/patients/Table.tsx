  ...
  return (
    <div className="rounded-2xl border overflow-hidden">
      <table className="w-full text-sm">
        ...
        <tbody>
          {loading && <tr><td colSpan={6} className="px-3 py-6 text-center">Cargando…</td></tr>}
          {!loading && rows.length === 0 && <tr><td colSpan={6} className="px-3 py-6 text-center">Sin resultados.</td></tr>}
          {rows.map((r) => (
            <tr key={r.id} className="border-t">
              ...
              <td className="px-3 py-2">{r.deleted_at ? new Date(r.deleted_at).toLocaleDateString() : "—"}</td>
            </tr>
          ))}
        </tbody>
      </table>
