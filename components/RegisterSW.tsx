...
export default function RegisterSW() {
  ...
  useEffect(() => {
    ...
    const onControllerChange = () => {
      // Cuando se activa el nuevo SW, recarga para tomarlo
    ...
    const registerNow = async () => {
      try {
        ...
        // Si ya hay uno esperando (p.ej. por refresh), ofrece actualizar
        if (reg.waiting) {
          waitingSW.current = reg.waiting;
          setUpdateReady(true);
        }
        // Detecta nuevos updates
        reg.addEventListener("updatefound", () => {
          const sw = reg.installing;
          if (!sw) return;
          sw.addEventListener("statechange", () => {
            // Cuando el nuevo SW termina de instalar y ya hay uno controlando la página → hay update
        ...
        // Escucha cuando el controlador cambia tras saltar el "waiting"
  ...
  // Banner minimalista y acorde al tema Sanoa
  return (
      ...
      <div className="mx-4 rounded-2xl border border-[var(--color-brand-border)] bg-white shadow">
        <div className="flex items-center justify-between gap-3 p-3">
          ...
          <div className="flex gap-2">
            <button
              onClick={() => (waitingSW.current = null, /**/ (document.activeElement as HTMLElement | null)?.blur(), /**/ (updateReady && false))}
