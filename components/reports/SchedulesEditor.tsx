...
export default function SchedulesEditor({ orgId }: { orgId: string }) {
  ...
  function validateTarget(): { ok: boolean; msg: string } {
    ...
    return isE164(norm)
      ? { ok: true, msg: "E.164 válido" }
      : { ok: false, msg: "Usa formato +<código><número>" };
  }

  async function save() {
    const vt = validateTarget();
    if (!vt.ok) {
      alert(`Destino inválido: ${vt.msg}`);
      return;
    }
          ...
          <div className="col-span-2">
            ...
            <p
              className={`text-xs mt-1 ${
                (form.target ?? "").trim()
                  ? form.channel === "email"
                    ? isEmail((form.target ?? "").trim())
                      ? "text-green-700"
                      : "text-rose-700"
                    : isE164(normalizeE164((form.target ?? "").trim()))
                      ? "text-green-700"
                      : "text-rose-700"
                  : "text-slate-500"
              }`}
            >
              {!(form.target ?? "").trim()
                ...
                : form.channel === "email"
                  ...
                  : isE164(normalizeE164((form.target ?? "").trim()))
                    ? "E.164 válido (+...)"
                    : "Formato inválido: +<código><número>"}
            </p>
          </div>
        </div>
