...
export default function RemindersFilters() {
  ...
  const [channel, setChannel] = useState<string[]>(initial.channel ? initial.channel.split(",") : []);
    ...
    <div className="rounded-2xl border p-4 space-y-3">
      <div className="grid grid-cols-1 md:grid-cols-5 gap-3">
        <div className="md:col-span-2">
          ...
          <input className="rounded border px-3 py-2 w-full" value={q} onChange={e => setQ(e.target.value)} placeholder="Destino, plantilla…" />
        </div>
        <div>
          <label className="block text-sm mb-1">Campo de fecha</label>
          <select className="rounded border px-3 py-2 w-full" value={dateField} onChange={e => setDateField(e.target.value)}>
